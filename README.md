...
這是一個提供高齡者查詢高雄市運動資源的靜態互動網站。使用者可以依照行政區、資源分類或關鍵字，快速找到附近的運動中心、特色公園、可運動學校、運動課程與運動園區。
...
- 行政區篩選（11個重點行政區）
- 資源分類（運動中心、公園、可運動學校、運動課程、運動／體育園區）
...
安裝 Live Server 延伸套件後，右鍵 `index.html`，選擇「Open with Live Server」。
...
2. 將 `kaohsiung-static/` 資料夾內的所有檔案（包含 `.nojekyll`）上傳至 repository 根目錄。
...
本專案所有檔案路徑均使用相對路徑（例如 `./data/venues.json`、`./style.css`），可在 GitHub Pages 的子目錄下正確運作，無需額外設定。
