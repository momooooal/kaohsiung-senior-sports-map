...
- 已刪除走跑高雄3.0參考圖片，網站僅保留文字介紹與官方連結。
- 運動中心、公園、運動i臺灣課程、運動／體育園區、行動健身房巡迴車、走跑高雄3.0及「顯示全部」均改用可愛、易辨識的專屬圖示。

## 第七版更新

- 已再次完整移除「可運動的學校」按鈕、資料檔、搜尋功能、卡片、圖示及相關樣式。
- 已加入版本快取更新設定，避免瀏覽器繼續顯示舊版首頁。

## 第八版更新

- 已完整刪除首頁「請選擇您所在的區域」整個區塊。
- 已移除行政區按鈕、行政區篩選條件及 `districts.json`。
- 使用者改以運動資源分類或搜尋欄查詢。
- 搜尋欄仍可輸入行政區名稱，例如「岡山區」或「前鎮區」，查找資料中實際存在的資源。


## 第九版更新

- 將「1. 您想找哪一種運動資源？」改為「🏃 您想找哪一種運動資源？」。
- 將「2. 搜尋運動地點」改為「🔎 也可以直接在搜尋欄輸入關鍵字，找到您想要的運動地點」。
- 兩個區塊皆不再使用數字編號。
...
- 每一張公園卡片均新增管理單位標註：
  「高雄市政府工務局公園處」。
- 新增「前往公園處網站」按鈕，供民眾查詢或反映公園相關問題。
- 公園處網站：https://pwbpo.kcg.gov.tw/News_Photo.aspx?n=368FF1FBF413E73A&sms=3B7F915DCFF56ABD
